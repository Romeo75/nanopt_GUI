function varargout = NoiseCorrection(varargin)
    ...
    %      stop.  All inputs are passed to NoiseCorrection_OpeningFcn via varargin.
    %
    ...
    gui_State = struct('gui_Name', mfilename, ...
        ...
        'gui_OpeningFcn', @NoiseCorrection_OpeningFcn, ...
        'gui_OutputFcn', @NoiseCorrection_OutputFcn, ...
        'gui_LayoutFcn', [], ...
        'gui_Callback', []);
...
function varargout = NoiseCorrection_OutputFcn(hObject, eventdata, handles)
...
function pushbutton2_Callback(hObject, eventdata, handles)
    ...
    handles.edit1.String = "select the right noise background...";
    handles.noise_left = drawrectangle('Color', 'r', 'Parent', handles.axes2);
    handles.edit1.String = "all noise background selected";
    handles.edit1.String = "select the left noise background...";
    handles.noise_right = drawrectangle('Color', 'r', 'Parent', handles.axes2);
    handles.edit1.String = "left noise background selected";
    % Update handles structure (variables of the running program)
    guidata(hObject, handles); % This saves all of the data saved in handles as variables for later use
    % Since Noice seems to break uiwait, it is called again.
    uiwait(handles.Noise);
...
function pushbutton3_Callback(hObject, eventdata, handles)
    ...
    left_intervall_start = position_left(1);
    left_intervall_end = position_left(1) + position_left(3);

    left_noise = counts(left_intervall_start < lambda & lambda < left_intervall_end);
    right_noise = counts(right_intervall_start < lambda & lambda < right_intervall_end);

    left_level = mean(left_noise);
    right_level = mean(right_noise);

    polynom_deriv = (right_level - left_level) / (right_intervall_start - left_intervall_start);
    polynom_origin = left_level - polynom_deriv * left_intervall_start;
    background_values = polyval([polynom_deriv polynom_origin], lambda);
